...
Une deuxième solution est de développer une application de bureau sous **[Python](https://www.python.org)** en utilisant soit le module *tKinter*, soit *PyQt5* pour se charger de l'interface utilisateur. L'avantage est d'avoir un **programme très léger et relativement simple de fonctionnement**. 

En revanche, l'organisation et le fait de devoir bricoler des fonctionnalités pour faire ce dont elles ne sont pas conçues pour à la base comme la recherche continue de code-barre sur un flux vidéo, m'ont orienté vers une autre solution.

| Avantages |  Inconvénients    |
|:---------------|:---------------|
|   Programmation simple.    |   Aucune fonctionnalité adapté au besoin sans avoir à trouver une façon de les implémenter. |
|   Open-source via la *Python Software Foundation (PSF)* et une documentation massive.    |   Structure mono-fichier nuisant à la lisibilité du projet.                                              |
|   Programme final très léger.    |   -                                                 |
Table: Avantages et inconvénients de la solution *Python GUI*.

#### Microsoft Visual Studio\

Une troisième solution concerne l'utilisation de Microsoft Visual Studio, logiciel de Microsoft permettant le développement de logiciels en langages *C++* et *C#*. **Ses fonctionnalités permettant de faire ce que l'on souhaite avec un minimum de contraintes**. Cependant celui-ci appartient à *Microsoft*, et ni l'entreprise Ondal, ni Mibelle Group, ne disposent de la licence; par conséquent impossible de l'utiliser.
...
La partie **editeur** sera basée sur une solution *Python GUI*, du fait de la simplicité et du besoin de générer un code QR. Il n'y a pas de nécessité de créer un outil permettant à la fois de générer et d'interpréter des étiquettes; cela alourdirait et ralentirait le programme final.
...
![Illustration des technologies employées dans la partie app.](assets/images/3+/app-logos.png)
 ...
 -  **Node.js** fournit des outils pour accéder à fonctions dont on n'aurait pas accès normalement, comme l'écriture de fichier sur disque, ou l'éxecution de commandes. De plus, il fournit un gestionnaire de modules permettant d'ajouter des fonctionnalités.\

Pour finir, on peut récupérer le flux vidéo d'une caméra en utilisant un *API*[^12] qui se nomme *Navigator*, et plus particulièrement *Navigator.mediaDevices*. En quelques lignes, on réussi à obtenir un flux vidéo avec des paramètres souhaités.
...
D'une façon générale, le développement de ces deux solutions a été fluide et sans réel soucis, tant en termes de compétences de programmation, que de soucis techniques. Cependant, certains points sont intéressants à relever;
 ...
 -  **Partie app**:
    ...
    3.  Sur un point plus technique, **l'API[^12] d'ElectronJS et de Node.js n'est disponible que pour le script de pré-chargement de l'application**, pour des questions de sécurité. Ainsi, il n'était pas possible d'utiliser les fonctionnalités de Node.js sur les pages de l'application, ce qui ne permettait pas le développement de celle-ci. La solution a été de **spécifier dans les paramètres d'ElectronJS l'autorisation de pouvoir utiliser son API depuis n'importe où**. Ça n'a pas été compliqué, mais j'y ai passé du temps. (cf. [**contextIsolation**](https://www.electronjs.org/docs/tutorial/context-isolation))
...
On verra dans la partie suivante que la solution répond parfaitement à la problématique. Cependant, j'estime que l'on peut toujours améliorer un produit. Ainsi, avec plus de temps et de moyens, il serait intéressant;
...
-   **Gain de performance et de poids**. Comme évoqué précédemment, la partie app est un site internet que l'on a integré comme une application de bureau. Pour gagner en performance et en poids, il est possible d'héberger ce site internet sur un serveur Node.js qui se charge du calcul, et une application traitant les informations depuis et vers la tablette.
